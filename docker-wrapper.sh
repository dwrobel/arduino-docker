...
if [ -e "${config_file}" ]; then
    ...
    # DOCKER_RUN=("--net=host" "-v" "/data:/data")
    source "${config_file}"
fi

DOCKER_IMG=dwrobel/arduino:30
...
${SUDO} docker run --net=host "${DOCKER_RUN[@]}" ${DOCKER_RUN_OPTS} --privileged -i ${USE_TTY} ${cache_dir} ${cc_opts} ${cxx_opts} ${wayland_display_opts} -e USER=$USER -e UID=$UID -e GID=$(id -g $USER) -e CWD="$CWD" ${display_opts} ${xdg_runtime_opts} -v /tmp/.X11-unix:/tmp/.X11-unix -v "${VDIR}":"${VDIR}" ${DOCKER_IMG} "$@"
